...
# Orquestra todos os módulos para o ambiente PRODUÇÃO
# Máxima segurança: Multi-AZ, GuardDuty + Config, deletion_protection, multi-NAT
...
# ── CloudTrail (auditoria global, multi-region) ───────────────────────────────
resource "aws_cloudwatch_log_group" "cloudtrail" {
  name              = "/aws/cloudtrail/${local.name_prefix}"
  retention_in_days = 365 # 1 ano de retenção em prod (PCI/SOC2)
}
...
resource "aws_cloudtrail" "this" {
  ...
  is_multi_region_trail         = true # obrigatório em prod — cobre todas as regiões
  ...
  event_selector {
    ...
    # Auditoria de acesso a objetos S3 em prod
    data_resource {
      type   = "AWS::S3::Object"
      values = ["${module.s3.app_bucket_arn}/"]
    }
  }
...
module "vpc" {
  ...
  # Prod usa NAT Gateway por AZ para alta disponibilidade
...
module "monitoring" {
  ...
  # Prod: GuardDuty + AWS Config obrigatórios
  enable_guardduty  = true
  enable_aws_config = var.enable_aws_config
  config_s3_bucket  = module.s3.logs_bucket_name
...
module "rds" {
  ...
  # Prod: Multi-AZ obrigatório para alta disponibilidade e failover automático
  multi_az = true
  ...
  backup_retention_days     = 30   # 30 dias de backup em prod
