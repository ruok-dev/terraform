...
# Orquestra todos os módulos para o ambiente DEV
...
resource "aws_cloudtrail" "this" {
  ...
  # S9: multi-region garante auditoria mesmo de chamadas de API em outras regiões
...
# ── Módulo S3 (inclui bucket de logs usado pelo CloudTrail) ──────────────────
module "s3" {
  source = "../../modules/s3"
  ...
  create_tfstate_bucket = false # state bucket já criado separadamente via bootstrap
}
...
module "vpc" {
  ...
  single_nat_gateway       = true # em dev usa 1 NAT para economizar custo
...
module "security" {
  ...
  # B1: Passa os IDs reais das route tables privadas (necessário para o S3 Gateway Endpoint funcionar)
...
module "monitoring" {
  ...
  # S2: Passa a chave KMS do S3 para criptografar os log groups do CloudWatch
  kms_key_arn               = module.s3.s3_kms_key_arn
